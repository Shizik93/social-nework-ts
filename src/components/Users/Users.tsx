                ...
                <div key={f.id}>
                    ...
                    {f.followed ?
                        <button onClick={() => {
                            ...
                            usersAPI.deleteFollow(f.id).then(resultCode => {
                                if(resultCode===0){props.UnFollow(f.id)}

                            })
                            }}>Unfollow</button> :
                        <button onClick={() => {
                            usersAPI.postFollow(f.id).then(resultCode => {
                            if(resultCode===0){props.Follow(f.id)}
